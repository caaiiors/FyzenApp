...
async function corrigirAssinaturas() {
  ...
  try {
    ...
    setLogs(prev => [...prev, "❌ Erro ao corrigir assinaturas (veja o console)."]);
  }
}
...
function PremiumUsersList() {
  ...
  if (loading)
    return (
      <p className="text-sm text-slate-400 py-2">Carregando usuários premium…</p>
    );

  if (!lista.length)
    return <p className="text-sm text-slate-500">Nenhum usuário premium encontrado.</p>;
    ...
    <div className="space-y-6">
      <header className="space-y-1">
        ...
        <p className="text-sm text-slate-400">
          Todas as métricas abaixo são calculadas em tempo real a partir das
          coleções <code>planos</code>, <code>historicoTreino</code> e{" "}
        ...
        <form onSubmit={handleSendNotification} className="space-y-3">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="mt-6 p-4 glass-card border border-white/10 rounded-xl max-h-80 overflow-y-auto text-sm space-y-1">
              <p className="text-teal-300 font-semibold mb-2">Logs da manutenção:</p>
              {logs.map((log, i) => (
